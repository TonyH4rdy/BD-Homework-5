...
conn = psycopg2.connect(database="hometaskdb", user="postgres", password="997111")
with conn.cursor() as cur:
    create_tables(cur)
    check_function(cur)
    add_new_client(cur, "Игорь", "Иванов", "ig@g.com")
    add_new_client(cur, "Андрей", "Петров", "an@g.com")
    add_new_client(cur, "Виктор", "Сергеев", "vi@g.com")
    add_new_client(cur, "Иван", "Дмитриев", "iv@g.com")
    add_new_client(cur, "Василий", "Галкин", "va@g.com")
    add_new_phonenumber(cur, 1, "123456789")
    add_new_phonenumber(cur, 2, "987654321")
    add_new_phonenumber(cur, 3, "132457689")
    add_new_phonenumber(cur, 4, "978653421")
    add_new_phonenumber(cur, 5, "123459876")
    change_client_data()
